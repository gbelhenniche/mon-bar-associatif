...
export default function FournisseursArchives({ archives }: Props) {
    ...
    function confirmRestore() {
        ...
        setProcessing(true);
        router.post(`/admin/fournisseurs-archives/${restoreTarget.id}/restore`, {}, {
    ...
    function confirmDelete() {
        ...
        setProcessing(true);
        router.delete(`/admin/fournisseurs-archives/${deleteTarget.id}`, {
    ...
    return (
        <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
            <div>
                <h1 className="font-display text-2xl md:text-3xl font-semibold">Fournisseurs archivés</h1>
            ...
            {filtered.length === 0 ? (
                <div className="text-center py-16 text-muted-foreground">
                    {search ? 'Aucun résultat.' : 'Aucun fournisseur archivé.'}
                </div>
            ) : (
            ...
            <Dialog open={!!restoreTarget} onOpenChange={open => { if (!open) setRestoreTarget(null); }}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Restaurer le fournisseur</DialogTitle>
                    </DialogHeader>
                    <p className="text-sm text-muted-foreground">
                        <strong>{restoreTarget?.nom}</strong> sera réintroduit dans la liste active des fournisseurs.
                    </p>
                    <DialogFooter>
            ...
            <Dialog open={!!deleteTarget} onOpenChange={open => { if (!open) setDeleteTarget(null); }}>
                <DialogContent>
                    ...
                    <p className="text-sm text-muted-foreground">
                        <strong>{deleteTarget?.nom}</strong> sera supprimé définitivement et ne pourra pas être récupéré.
                    </p>
                    <DialogFooter>
...
FournisseursArchives.layout = (page: React.ReactNode) => <AuthenticatedLayout>{page}</AuthenticatedLayout>;
