...
export default function Categories({ categories, types }: Props) {
    ...
    const saveCat = () => {
        ...
        if (catEditing.id) {
            router.put(`/categories/${catEditing.id}`, payload, {
    ...
    const saveType = () => {
        ...
        if (typeEditing.id) {
            router.put(`/types-materiel/${typeEditing.id}`, payload, {
                onSuccess: () => { toast.success('Mis à jour'); setTypeOpen(false); setTypeEditing(emptyType); },
                onError: () => toast.error('Erreur lors de la mise à jour'),
            });
        } else {
            router.post('/types-materiel', payload, {
    ...
    const deleteType = (id: number) => {
        ...
        router.delete(`/types-materiel/${id}`, {
    ...
    const moveType = (t: MType, dir: -1 | 1) => {
        router.put(`/types-materiel/${t.id}`, {
